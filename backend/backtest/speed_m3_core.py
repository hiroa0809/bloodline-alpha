...
def build_trip_arrays(a: dict, runs: list[dict], ana_end: int) -> list[tuple[str, str]]:
    ...
    n = len(targets)
    # 整合ガードは計測装置の最重要不変条件。python -O で除去される assert ではなく
    # 明示 raise で実行時に必ず効かせる（サイレント行ズレ＝別馬への紐づけを確実に検出）。
    if n != len(a["chaku"]):
        raise ValueError(
            f"M3 targets が M2 配列と整合しません（件数不一致 {n} != {len(a['chaku'])}）"
        )
    ...
    if not (
        np.array_equal(m3_rid, a["_row_race_id"])
        and np.array_equal(m3_uma, a["_row_umaban"])
    ):
        raise ValueError(
